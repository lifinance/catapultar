 ...
 * @notice Tron variant of CATValidator. Uses a SafeTransferLib wrapper that bypasses
 * broken transfer() return values (Tron USDT) via approve(self) + transferFrom(self).
 *
 * Validation of a pre-approved asset allowance to execute a transaction that should result in a specific asset
 * outcome.
 ...
 */
contract CATValidatorTron is EIP712, ReentrancyGuard {
