...
describe("CatapultarTx", () => {
  ...
  it("should be able to validate a provided signature is valid", async () => {
    const key = random(32);
    ...
    expect(await tx.hasValidSignature({ noSignatureIsValid: false })).toBe(false);
    expect(await tx.hasValidSignature({ noSignatureIsValid: true })).toBe(true);
  ...
  it("should allow you to BYO signer", async () => {
    ...
    await tx.sign(({domain, types, primaryType, message}) => account.signTypedData({domain, types, primaryType, message}));
    ...
    expect(await tx.hasValidSignature({ noSignatureIsValid: false })).toBe(true);
  });
  ...
  it("should return proper getOpData", async () => {
    ...
    const opDataWithSignature = await tx.getOpData({ compactSignature: false });
    ...
    const opDataWithCompactSignature = await tx.getOpData({ compactSignature: true });
...
describe("MetaCatapultarTx", () => {
  it("should correctly set nonces for calls", async () => {
  ...
  it("should validate that nonces are not dublicates", () => {
    ...
    mTx.addCalls(...calls);
    expect(() => mTx.checkNonces()).toThrow("Dublicate nonces were found: 5,5")
    expect(() => mTx.asCatapultarTx()).toThrow("Dublicate nonces were found: 5,5")
  });
});
...
//         (
//           await provider.broadcastTransaction(toBroadcast)
//         ).hash
//       );
...
//         nonce: executorNonce++,
//       });
//       const toBroadcastTransaction = await executor1.signTransaction(
//         toExecuteTransaction
//       );
//       await provider.getTransactionReceipt(
//         (
//           await provider.broadcastTransaction(toBroadcastTransaction)
//         ).hash
//       );
